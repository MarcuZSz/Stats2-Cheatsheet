...
Modelliert Anzahl Versuche bis A zum 1. Erfolg eintritt\\
...
\item Wenn gefragt: $Z=\frac{\hat{p} - p}{SE}$
\end{enumerate}
...
Gegeben: $P(L) = 0,02$, $P(Pos|L)=0,98$, $P(\overline{Pos}|\bar{L})=0,74$

\begin{align*}
& P (\bar{L}) = 1 - P(L) \\
& P (Pos|\bar{L}) = 1 - P(\overline{Pos}|\bar{L}) \\
...
\item Erwartungswerte berechnen: $E_{xy}=\frac{h_{x} \cdot h_{y}}{n}$
\item $\chi^2$ berechnen
\item Freiheitsgrad berechnen: $df = (n_x-1)(n_y-1)$
