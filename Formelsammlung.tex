...
\[
P(X=x)=f(x)=\binom{n}{k} \cdot p^k(1-p)^{n-k}
\]
...
\[\resizebox{0.2\textwidth}{!}{$
f(x)=\binom{n-1}{k-1}p ^k(1-p)^{n-k}|(n \geq k), p \in (0,1)
$}
\]
...
\[
f(x)= \underbrace{(1 - p)^{n-1}}_{\text{(n-1)-mal } \overline{A}}
 \cdot \underbrace{p}_{\text{1-Mal A}}, p \in (0,1)
\]
...
\[
f(x)=exp(-\lambda)\frac{\lambda^k}{k!}
\]
\[
...
\includegraphics[scale=0.25]{Bilder/1.png} 
\includegraphics[scale=0.25]{Bilder/2.png} 
